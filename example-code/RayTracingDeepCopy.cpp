...
struct MT_RNG {
    ...
    MT_RNG() {
        for (int i = 0; i < NUM_THREADS; ++i) rngs[i].seed(time(nullptr) + i);
    };
...
inline void loadObj(Scene *scene, const int material, const std::string &meshPath) {
    /// Helpers
...
inline void buildBVHTree(Scene *scene) {
...
inline Vec render(const Scene *scene, const int x, const int y, const int spp) {
    Vec colour{};
...
inline Scene* loadScene() {
    ...
    scene->camera = Camera(Vec{ 0., 500., -1700. }, Vec{ 0., 0., 1. }.normal(), 42.501, 1024, 1024);

    scene->materials.push_back(Material{ { .9,   .9,  .9 }, {   0,   0,   0 } }); // White
    scene->materials.push_back(Material{ { .81, .23, .14 }, {   0,   0,   0 } }); // Red
    scene->materials.push_back(Material{ { .23, .41, .24 }, {   0,   0,   0 } }); // Green
    scene->materials.push_back(Material{ { .62, .71, .13 }, {   0,   0,   0 } }); // Yellow
    scene->materials.push_back(Material{ {  0.,  0.,  0. }, { 100, 100, 100 } }); // Light Source

    loadObj(scene, 0, "assets/cornellbox-white.obj");
    loadObj(scene, 1, "assets/cornellbox-red.obj");
    loadObj(scene, 2, "assets/cornellbox-green.obj");
    loadObj(scene, 3, "assets/bunny.obj");
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        for (int i = 0; i < bw * bh; ++i) {
            ...
            const Vec col = render(scene, (bx + x), (by + y), spp);
